    ...
    <transition name="show">
      <div id="login_page" v-show="loginform">
        <div id="login_form" v-show="!turn_to_reg">
          ...
          <div class="password">
            密码：<el-input name="login_psw" placeholder="请输入密码" v-model="login_psw"></el-input>
          </div>
          <div class="checkbox"><el-checkbox v-model="remember">记住我</el-checkbox></div>
          <div class="login-button"><button id="login">登录</button></div>
    ...
    checkCard: function () {
      ...
      this.$validator.validate('reg_name').then(result => {
        if (result) {
          axios({
            method: 'post',
            url: 'http://localhost:8082/checkUsername',
            headers: {
              'Content-type': 'application/x-www-form-urlencoded'
            },
            params: {
